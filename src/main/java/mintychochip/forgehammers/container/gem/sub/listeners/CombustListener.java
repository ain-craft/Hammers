...
public class CombustListener extends AbstractListener {

  public CombustListener(ForgeHammers instance) {
    super(instance);
  }
  private final Grasper<ItemMeta, GemContainer> grasper = new Grasper<>() {
  };
  @EventHandler
  private void onDestroy(final EntityDamageEvent event) {
    ...
    for (ExecutionPriority value : ExecutionPriority.values()) {
      grab.keySet().stream().map(gem -> Gem.getGem(gem.getNamespace()))
          .filter(gem -> gem instanceof ITriggerOnDamageEvent).map(gem -> (ITriggerOnDamageEvent) gem)
  ...
  @EventHandler
  private void combust(final EntityCombustEvent event) {
    Entity entity = event.getEntity();
    if(!(entity instanceof Item item)) {
      return;
    }
    ...
    for (ExecutionPriority value : ExecutionPriority.values()) {
      grab.keySet().stream().map(gem -> Gem.getGem(gem.getNamespace()))
          .filter(gem -> gem instanceof ITriggerOnCombust).map(gem -> (ITriggerOnCombust) gem)
          .filter(triggerOnBlockDrop -> triggerOnBlockDrop.getPrio() == value)
          .forEach(triggerOnBlockDrop -> triggerOnBlockDrop.execute(event,
              grab.getLevel(((Gem) triggerOnBlockDrop).getGemEnum())));
    }
